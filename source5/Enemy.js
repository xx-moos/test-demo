...
export default class Enemy {
    ...
    draw(ctx, pause, pacman) {
      ...
      const enemyRow = Math.floor(this.y / this.tileSize);
      const enemyCol = Math.floor(this.x / this.tileSize);
      const distance = Math.abs(enemyRow - pacmanRow) + Math.abs(enemyCol - pacmanCol);
      const viewRadius = 3;

      // If the enemy is within the field of view, draw the enemy
