...
const clones = tileMap.getClones(tileSize, velocity); // create clones
const testClone = new Clone(0, 0, tileSize, velocity, tileMap);
console.log(testClone); // for some reason clone.js cant be read and all clone can do is eat dots (testClone resolution given by Chatgpt)
...
function gameLoop() {
    ...
    clones.drawClones(ctx); //Draw all clones
    clones.moveClones(enemies);  // Move all clones to check the enemies
...
// create clone when the power dot has been eaten
pacman.onEatPowerDot = () => {
  clones.addClone(pacman.x, pacman.y); // Spawns a clone at Pacman's current position
};
