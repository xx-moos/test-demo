...
const gameOverSound = new Audio("sounds8/gameOver.wav");
const gameWinSound = new Audio("sounds8/ChillGuy.mp3");

// redraw the screen a certain number of time per second
function gameLoop() {
    tileMap.draw(ctx); // Draw the tilemap\
    drawGameEnd();
    pacman.draw(ctx, pause(),); // keep drawing the pacman and pass it to ctx value / otherwise pause / eat a ghost
